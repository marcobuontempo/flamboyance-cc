...
export default function Trades() {
  ...
  const columns: ColumnDef<TransformedLiveDataTrade>[] = [
    ...
    {
      header: `From Amount (${preferredCurrency})`,
    ...
    {
      header: `To Amount (${preferredCurrency})`,
      accessorKey: 'to_amount_fiat',
      cell: info => <GreenTextCell value={info.getValue() as string} />,
    },
    {
      header: `Total Fees (${preferredCurrency})`,
  ...
  const transformData = (entry: LiveDataTrade): TransformedLiveDataTrade => {
    ...
    return {
      ...
      from_amount_fiat: (entry.from_amount_usd * exchangeRate).toLocaleString('en-US', { style: 'currency', currency: preferredCurrency }),
      to_combined: toTokenData,
      to_amount: formatRawAmountToDecimals(parseInt(entry.to_amount), toTokenData?.decimals),
      to_amount_fiat: (entry.to_amount_usd * exchangeRate).toLocaleString('en-US', { style: 'currency', currency: preferredCurrency }),
      total_fees_fiat: (entry.total_usd_fees * exchangeRate).toLocaleString('en-US', { style: 'currency', currency: preferredCurrency }),
    };
  };
