...
export default function LiquidityPools() {
  ...
  const columns: ColumnDef<TransformedLiveDataLiquidityPool>[] = [
    ...
    {
      header: `LP Amount (${preferredCurrency})`,
    ...
    {
      header: `Token 1 Amount (${preferredCurrency})`,
    ...
    {
      header: `Token 2 Amount (${preferredCurrency})`,
  ...
  const transformData = (entry: LiveDataLiquidityPool): TransformedLiveDataLiquidityPool => {
    ...
    return {
      ...
      lp_fiat_amount: (entry.lp_usd_amount * exchangeRate).toLocaleString('en-US', { style: 'currency', currency: preferredCurrency }),
      token_1_combined: token1Data,
      token_1_amount: formatRawAmountToDecimals(parseInt(entry.token_1_amount), token1Data?.decimals),
      token_1_fiat_amount: (entry.token_1_usd_amount * exchangeRate).toLocaleString('en-US', { style: 'currency', currency: preferredCurrency }),
      token_2_combined: token2Data,
      token_2_amount: formatRawAmountToDecimals(parseInt(entry.token_2_amount), token2Data?.decimals),
      token_2_fiat_amount: (entry.token_2_usd_amount * exchangeRate).toLocaleString('en-US', { style: 'currency', currency: preferredCurrency }),
    };
  };
